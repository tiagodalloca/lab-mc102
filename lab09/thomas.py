...
for i in range(4):
    ...
    linha = []
'''------------------------------------------------------------------------------------------------------------'''
# acha todas as possibidades de compra para todas as empresas, desde que o
# lucro seja positivo
possCompra = []
aux = []
a = Compra(-1, -1, -1, -1)
...
if(len(possCompra) != 0):
    ...
    for i in range(len(possCompra)):
        for j in range(len(possCompra[i])):
            ...
            aux.append(
                Compra(
                    possCompra[i][j].getEmpresa(),
                    possCompra[i][j].getDtCompra(),
                    possCompra[i][j].getDtVenda(),
                    possCompra[i][j].getLucro()))  # adiciona compra ao negocio
            possNegocio.append(aux[:])  # guarda o negocio atual

            for k in range(len(possCompra)):
                for w in range(len(possCompra[k])):
                    # se a compra verificada ainda nao existe no negocio e se
                    # ela nao da conflito de data com outra compra ja inserida
                    # no negocio
                    if(possCompra[i][j].getEmpresa() != possCompra[k][w].getEmpresa() and (possCompra[i][j].getDtVenda() <= possCompra[k][w].getDtCompra() or possCompra[k][w].getDtVenda() <= possCompra[i][j].getDtCompra())):
                        aux.append(
                            Compra(
                                possCompra[k][w].getEmpresa(),
                                possCompra[k][w].getDtCompra(),
                                possCompra[k][w].getDtVenda(),
                                possCompra[k][w].getLucro()))  # adiciona compra ao negocio
                        possNegocio.append(aux[:])  # guarda o negocio atual

                        for e in range(len(possCompra)):
                            for r in range(len(possCompra[e])):
                                # se a compra verificada ainda nao existe no
                                # negocio e se ela nao da conflito de data com
                                # outra compra ja inserida no negocio
                                if(possCompra[e][r].getEmpresa() != possCompra[k][w].getEmpresa() and possCompra[e][r].getEmpresa() != possCompra[i][j].getEmpresa() and
                                   ...
                                   (possCompra[e][r].getDtVenda() <= possCompra[i][j].getDtCompra() or possCompra[i][j].getDtVenda() <= possCompra[e][r].getDtCompra())):
                                    aux.append(
                                        Compra(
                                            possCompra[e][r].getEmpresa(),
                                            possCompra[e][r].getDtCompra(),
                                            possCompra[e][r].getDtVenda(),
                                            possCompra[e][r].getLucro()))  # adiciona compra ao negocio
                                    # guarda o negocio atual
                                    possNegocio.append(aux[:])

                                    for t in range(len(possCompra)):
                                        for y in range(len(possCompra[t])):
                                            # se a compra verificada ainda nao
                                            # existe no negocio e se ela nao da
                                            # conflito de data com outra compra
                                            # ja inserida no negocio
                                            if(possCompra[t][y].getEmpresa() != possCompra[e][r].getEmpresa() and possCompra[t][y].getEmpresa() != possCompra[k][w].getEmpresa() and
                                               ...
                                               (possCompra[e][r].getDtVenda() <= possCompra[t][y].getDtCompra() or possCompra[t][y].getDtVenda() <= possCompra[e][r].getDtCompra())):
                                                aux.append(
                                                    Compra(
                                                        possCompra[t][y].getEmpresa(),
                                                        possCompra[t][y].getDtCompra(),
                                                        possCompra[t][y].getDtVenda(),
                                                        possCompra[t][y].getLucro()))  # adiciona compra ao negocio
                                                # guarda o negocio atual
                                                possNegocio.append(aux[:])
                                                # remove a ultima compra do
                                                # negocio, para testar a
                                                # proxima possibilidade
                                                aux = aux[:-1]
                                    # remove a ultima compra do negocio, para
                                    # testar a proxima possibilidade
                                    aux = aux[:-1]
                        # remove a ultima compra do negocio, para testar a
                        # proxima possibilidade
                        aux = aux[:-1]

    # apos encontrar todos os negocios, verifica qual deles é o melhor (maior
    # lucro)
    ...
    for i in range(len(possNegocio[melhor])):
        print(
            "acao %d: compra %d, venda %d, lucro %.2f" %
            (possNegocio[melhor][i].getEmpresa(),
             possNegocio[melhor][i].getDtCompra(),
             possNegocio[melhor][i].getDtVenda(),
             possNegocio[melhor][i].getLucro()))
